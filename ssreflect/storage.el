...
(defun read-lemmas ()
  (if (file-exists-p (concat home-dir "ssreflect/lemmas.txt"))
      (with-temp-buffer
	(insert-file-contents (concat home-dir "ssreflect/lemmas.txt"))
	(let ((temp (format "%s" (read (current-buffer)))))
	  (setf theorems_id (extract-info-from-files temp))
	  ))))

(defun read-views ()
  (if (file-exists-p (concat home-dir "ssreflect/views.txt"))
      (with-temp-buffer
	(insert-file-contents (concat home-dir "ssreflect/views.txt"))
	(let ((temp (format "%s" (read (current-buffer)))))
